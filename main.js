...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (loginForm && supabaseClient) {
        loginForm.addEventListener('submit', async function(e) {
            ...
            if (error) {
                if (error.message.includes('Email not confirmed')) alert('Please check your email and verify your account first.');
    ...
    if (signupForm && supabaseClient) {
        signupForm.addEventListener('submit', async function(e) {
            ...
            const { error } = await supabaseClient.auth.signUp({
                email: email, password: password, options: { data: { companyName: companyName } }
            });
    ...
    // 3. STRICT ADMIN 2-STEP AUTHENTICATION
    ...
    if (adminCredentialsForm && supabaseClient) {
    ...
    if (adminOtpForm && supabaseClient) {
        adminOtpForm.addEventListener('submit', async function(e) {
            ...
            const { error } = await supabaseClient.auth.verifyOtp({ email: email, token: otp, type: 'email' });
